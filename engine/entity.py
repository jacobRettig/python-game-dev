...
from engine.spriteSheetLPC import AnimationLPC
from engine.square import Square
import engine.util, sys
...
class Mob(Entity):
    def __init__(self, world, dim, direction=0, speed=.6, turnRate=math.pi/15, visDistance=50, visAng=math.pi/2, hearDistance=30,
         hp=20):
    ...
    def doActionTrigger(self):
        if self.act == -1:
            for i in range(len(self.acts)):
                print(self.acts[i].trigger(self))
                if self.acts[i].trigger(self) == True:
                    self.action = i
                    break
    ...
    @engine.util.InstanceGuard('_visVec', 'set')
    def visVec(self, val):
        self._visVec = Entity.normVector(val)
        
#     Make read-only
    visL = engine.util.InstanceGuard('_visL', None)
    visR = engine.util.InstanceGuard('_visR', None)
